...
def text_preprocessing(df2):
     ...
     # Regular expression to split hashtags into words
    pattern = r'#(\w+)'

    # Apply the regular expression on the 'tweet' column
    # df2['tweet'] = df2['tweet'].str.replace(pattern, lambda match: ' '.join(re.findall(r'[A-Z]?[a-z]+|[A-Z]+(?=[a-z]|\b)', match.group(1))))
    # remove number
    df2['final'] = df2['tweet'].map(lambda x: re.sub(r'\d+', '', x))
    # Remove HTTP tags
    df2['final'] = df2['final'].map(lambda x : ' '.join(re.sub("(@[A-Za-z0-9]+)|([^0-9A-Za-z \t])|(\w+:\/\/\S+)"," ",x).split()))
    #Lower Case
    ...
    # remove number
    df2['final'] = df2['final'].map(lambda x: re.sub(r'\d+', '', x))
    # it will remove all numeric terms in the tweet. 
    df2['final'] = df2['final'].map(lambda x: re.sub(r'\b\d+\b(?:\.\d+)?(?:[kmb]|\b)', '', x))
    # remove numberwithtext into text
    df2['final'] = df2['final'].map(lambda x: re.sub(r'\b\d+\b', '', x))
    df2['final'] = df2['final'].map(lambda x: re.sub(r'[^\w\s]', '', x))
    #Remove unicodes
    df2['final'] = df2['final'].map(lambda x : re.sub(r'[^\x00-\x7F]+',' ', x))
    # removes all non -alphbetical chars
    df2['final'] = df2['final'].map(lambda x: re.sub('[^a-zA-Z ]+', '', x))
    # convert to singular 
    # df2['final']  =df2['final'].map(lambda x : re.sub(r's\b','', x))
    # df2['final']  =df2['final'].map(lambda x : re.sub(r'ies\b', 'y', x))
    # df2['final']  =df2['final'].map(lambda x : re.sub(r'(us|ss)\b', r'\1e', x))
    # df2['final']  =df2['final'].map(lambda x : re.sub(r'(s|x|z|ch|sh)\b', r'\1', x))
    preprocessed_review = preprocess_text(df2['final'].values)
...
def read_dataset(args, vocab_path, MAX_SEQUENCE_LENGTH):
    ...
    if args.sentiment_data_path:
        ...
        df_sentiment_train, df_sentiment_test = df_sentiment.iloc[:int(
            2*len(df_task)), :], df_sentiment.iloc[int(0.99*len(df_sentiment)):, :]
    ...
    data_tokens, train_chars, ruling_embedding_train, category_embedding_train = get_indices(
        data_all.tweet, vocab, args.word_list_path)  # 
