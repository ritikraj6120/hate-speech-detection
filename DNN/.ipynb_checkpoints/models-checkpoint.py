...
def create_model(args, overal_maxlen, ruling_dim, vocab, num_class):
    ...
    if args.model_type == 'cls':
        ...
        embedded_sequences_word = Embedding(len(vocab), args.emb_dim, name='word_emb')(sequence_input_word)
        ruling_input = Input(shape=(ruling_dim,), dtype='float32',name='rule_input')
        emb_ruling = Embedding(len(vocab), 100,name='rule_emb')(ruling_input)
        ...
        emb_output = Embedding(len(vocab), args.emb_dim, name='word_emb')(input)
        # mlp_output = Self_Attention(300)(emb_output)
        mlp_output = MultiHeadAttention(300)(emb_output)
        mlp_output = Dense(300, activation='relu')(mlp_output)# mlp_output = Dropout(0.2)(mlp_output)
    ...
    if args.emb_path and args.model_type not in {'FNN', 'CNN', 'HHMM'}:
        ...
        # model.get_layer(name='word_emb').set_weights(emb_reader.get_emb_matrix_given_vocab(vocab, model.get_layer(name='word_emb').get_weights()))  # 升级至2.0.8
