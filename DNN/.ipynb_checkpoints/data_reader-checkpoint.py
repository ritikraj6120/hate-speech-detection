...
def read_dataset(args, vocab_path, MAX_SEQUENCE_LENGTH):
    ...
    df_task=df_task.head(100)
    df_task_test = pd.read_csv(args.trial_data_path, encoding="latin-1")
    df_task_test=df_task_test.head(100)
    df_task['task_idx'] = [0]*len(df_task)
    # the 'df_train.csv' means SE datasets.
    if args.data_path.split('/')[-1] != 'df_train.csv':
    ...
    category_embedding_train = pad_sequences(
        category_embedding_train, maxlen=MAX_SEQUENCE_LENGTH)
    category_embedding_test = pad_sequences(
        category_embedding_test, maxlen=MAX_SEQUENCE_LENGTH)
...
def get_data(args):
    ...
    X_train_data, X_test_data, y_train, y_test, train_chars, test_chars, task_idx_train, task_idx_test, train_ruling_embedding, test_ruling_embedding, category_embedding_train, category_embedding_test, vocab = read_dataset(args, args.vocab_path, args.maxlen)
