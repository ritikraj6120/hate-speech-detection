...
train_x, test_x, train_y, test_y, train_chars, test_chars, task_idx_train, task_idx_test, ruling_embedding_train, ruling_embedding_test,category_embedding_train, category_embedding_test, vocab = dataset.get_data(args)
print('**********************************************************************************************')
...
model = create_model(args, args.maxlen, len(ruling_embedding_test[0]),len(category_embedding_train[0]), vocab, len(train_y[0]))
...
evl = Evaluator(args, dataset, out_dir, test_x, test_chars, task_idx_test, ruling_embedding_test ,category_embedding_test, test_y, args.batch_size)
...
for ii in range(args.epochs):
    ...
    if args.model_type in {'CNN'}:
        ...
        #  ruling_embedding_train,category_embedding_train
        # , ruling_embedding_test,category_embedding_test
    elif args.model_type in {'HHMM', 'HHMM_transformer'}:
        train_history = model.fit([train_x, task_idx_train], train_y, batch_size=args.batch_size, epochs=1, validation_data=([test_x, task_idx_test], test_y), verbose=1)
    else:
