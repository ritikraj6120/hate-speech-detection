...
class Evaluator():
    ...
    def __init__(self, args, dataset, out_dir, test_x, test_chars, task_idx_test, ruling_embedding_test, category_embedding_test, test_y, batch_size):
        self.dataset = dataset
        self.model_type = args.model_type
        self.out_dir = out_dir
        self.test_x, self.test_chars, self.task_idx_test, self.ruling_embedding_test, self.category_embedding_test, self.test_y = test_x, test_chars, task_idx_test, ruling_embedding_test, category_embedding_test, test_y
        self.test_y = test_y
    ...
    def evaluate(self, model, epoch, print_info=False):
        ...
        if self.model_type in {'CNN'}:
            loss, acc = model.evaluate(
                self.test_chars, self.test_y, batch_size=self.batch_size, verbose=0)
            self.test_pred = model.predict(
                self.test_chars, batch_size=self.batch_size)
        elif self.model_type in {'HHMM_transformer'}:
            loss, acc = model.evaluate(
                [self.test_x, self.task_idx_test], self.test_y, batch_size=self.batch_size, verbose=0)
            self.test_pred = model.predict(
                [self.test_x, self.task_idx_test], batch_size=self.batch_size)
        else:
            loss, acc = model.evaluate(
                self.test_x, self.test_y, batch_size=self.batch_size, verbose=0)
            self.test_pred = model.predict(
                self.test_x, batch_size=self.batch_size)

        self.test_pred_label = np.argmax(self.test_pred, axis=1)
        self.test_y_label = np.argmax(self.test_y, axis=1)
        hs_pred, hs_y = get_hateSpeech(
            self.task_idx_test, self.test_y_label, self.test_pred_label)
        ...
        self.f1_all = f1_score(
            self.test_y_label, self.test_pred_label, average='macro')
        ...
        if print_info:
            ...
            logger.info(
                "Evaluation on test data: loss = %0.6f accuracy = %0.2f%%" % (loss, acc * 100))
            self.print_info()

    def print_info(self):
        logger.info("Evaluation on test data: acc = %0.6f " % self.acc)
        logger.info("Evaluation on test data: f1_hs = %0.6f " % self.f1_hs)
        logger.info("Evaluation on test data: f1_hs_wei = %0.6f " %
                    self.f1_hs_wei)
